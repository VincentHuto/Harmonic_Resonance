...
public enum EnumModArmorTiers implements IArmorMaterial {
	...
	NULL(HarmonicResonance.MOD_ID + ":null", 37, new int[] { 3, 6, 8, 3 }, 15, SoundEvents.ITEM_ARMOR_EQUIP_GENERIC,
			3.0F, 0.1F, () -> {
				return Ingredient.fromItems(ItemInit.null_ingot.get());
			}),
	CIRCLET(HarmonicResonance.MOD_ID + ":vibrational_seer", 37, new int[] { 1, 1, 1, 1 }, 15,
			SoundEvents.ITEM_ARMOR_EQUIP_GENERIC, 3.0F, 0.1F, () -> {
				return Ingredient.fromItems(ItemInit.null_ingot.get());
			}),
	MASK(HarmonicResonance.MOD_ID + ":mysterious_mask", 37, new int[] { 1, 1, 1, 1 }, 15,
			SoundEvents.ITEM_ARMOR_EQUIP_GENERIC, 3.0F, 0.1F, () -> {
				return Ingredient.fromItems(ItemInit.null_ingot.get());
			}),
	ELDER(HarmonicResonance.MOD_ID + ":elder", 37, new int[] { 1, 2, 3, 1 }, 15,
			SoundEvents.ITEM_ARMOR_EQUIP_GENERIC,3.0F, 0.1F, () -> {
				return Ingredient.fromItems(Blocks.BLACK_WOOL);
			});
	;
