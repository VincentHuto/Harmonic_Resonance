...
public class TileEntityKarmicAltar extends TileVibeSimpleInventory implements TickableBlockEntity, IExportableTile {
	...
	public void tick() {
		...
		// After the cooldown counter is done consume the item and add vibratory
		if (cooldown == 0) {
		...
		if (cooldown > 0) {
			if (count % 10 == 0) {
				...
				if (world.isRemote) {
					...
					HutosLibPacketHandler.sendLightningSpawn(vecabove, belowVec, 15f,
