...
/**
 * Created by TGG on 29/11/2015. Utility class to smooth out the motion of a
 * needle on a dial. Uses System.nanoTime() for timer.
 *
 * Usage: 1) Initialise the class with the desired acceleration and maximum
 * needle movement speed 2) setTargetNeedlePosition() whenever the needle has a
 * new target position. 3) getSmoothedNeedlePosition() to get the current needle
 * position
 *
 */
public class SmoothNeedleMovement {
	 ...
	 * @param instant              if true - move to new position instantly,
	 *                             otherwise smoothed.
	 */
	...
	// position = initial position + initial speed * time + 0.5 * acceleration *
	// time * time
	// 2) during maximum velocity phase:
	// speed = constant
	// position = speed * time
	// 3) deceleration is the same formula as (1) except that the acceleration is
	// reversed.
	...
	private void updateToCurrentTime(double elapsedTime) {
		...
		double accelerationTime = (remainingTime > accelerationPhaseDuration) ? accelerationPhaseDuration
				: remainingTime;
		remainingTime -= accelerationTime;

		needlespeed = initialNeedleSpeed + accelerationTime * needleAcceleration;
		needleposition = initialNeedlePosition + initialNeedleSpeed * accelerationTime
				+ 0.5 * needleAcceleration * accelerationTime * accelerationTime;
		// if we are past the acceleration phase, calculate for the steady movement
		// phase
		...
		// if we are past the steady movement phase, calculate for the decelerating
		// phase
		if (remainingTime > ARBITRARY_SMALL_DELTA) {
			double decelerationTime = (remainingTime > decelerationPhaseDuration) ? decelerationPhaseDuration
					: remainingTime;
			remainingTime -= decelerationTime;
			needleposition += needlespeed * decelerationTime
					- 0.5 * needleAcceleration * decelerationTime * decelerationTime;
	...
	/**
	 * Calculate the parameters for the needle to move to its new position. The
	 * needleposition and needlespeed must be current.
	 * 
	 * @param timeNowNS
	 */
	private void calculateMovementTimepoints(long timeNowNS, double newNeedlePosition, boolean instantMovement) {
		...
		// first need to calculate whether the needle will reach steady state speed or
		// not
		...
		// if the duration of the steadyphase is less than zero, we will overshoot! must
		// shorten the accel and decel phases
		if (steadyTime < 0) {
