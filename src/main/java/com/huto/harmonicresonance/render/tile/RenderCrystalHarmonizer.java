...
public class RenderCrystalHarmonizer extends BlockEntityRenderer<TileEntityCrystalHarmonizer> {
	private final ModelDrumMagatama magatamas = new ModelDrumMagatama();

	public RenderCrystalHarmonizer(BlockEntityRenderDispatcher rendererDispatcherIn) {
		super(rendererDispatcherIn);
	}

	@SuppressWarnings("deprecation")
	@Override
	public void render(TileEntityCrystalHarmonizer te, float partialTicks, PoseStack matrixStackIn,
		...
		for (int i = 1; i < te.getLevel(); i++) {
			...
			IVertexBuilder ivertexbuilder = irendertypebuffer$impl.getBuffer(magatamas
					.getRenderType(new ResourceLocation(HarmonicResonance.MOD_ID + ":textures/block/end_portal.png")));
